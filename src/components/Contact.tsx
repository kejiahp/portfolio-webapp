...
import { submitContactForm } from "../utils/axios-utils";
...
const Contact = () => {
  ...
  const { isLoading, mutate } = useMutation(
    (data: ContactFormInput) => submitContactForm(data),
    {
      onSuccess: () => {
        toast.success("You have successfully contacted Morenikeji😊😊", {
      ...
      onError: () => {
        toast.error("Error encountered, you can't contact elijah🥲😭", {
          icon: false,
          bodyClassName: "toastBody",
        });
      },
  ...
  const onSubmitHandler = (data: ContactFormInput) => {
    mutate(data);
  };
